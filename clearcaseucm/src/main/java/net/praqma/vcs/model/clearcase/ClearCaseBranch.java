...
public class ClearCaseBranch extends AbstractBranch {
	...
	/**
	 * Determines whether to care if one or the other part of the ClearCase
	 * branch fails its initialization
	 */
	...
	/**
	 * This Update implementation only updates the input view, because the
	 * output view does not need to be updated, only when demanded.
	 * 
	 * @author wolfgang
	 * 
	 */
	public class UpdateImpl extends Update {
		...
		public boolean update() {
			try {
				input.getSnapshotView().Update( true, true, true, false, new LoadRules( input.getSnapshotView(), Components.ALL ) );
